...
def generate_config_json(base_env : str,
                         num_envs : int = 0,
    ...
    for idx, var in enumerate(change_vars):
        if change_types[idx] is bool:
            ...
            var_values[var] = [val*(change_ranges[idx][1] - change_ranges[idx][0])//num_envs+change_ranges[idx][0] for val in range(num_envs)]
...
if __name__ == "__main__":
    ...
    try:
        assert case2 == generate_config_json( "MiniGrid-SimpleCrossingS9N0-v0",3,["num_crossings"],[int],[(1,4)])
    except:
        print(generate_config_json( "MiniGrid-SimpleCrossingS9N0-v0",3,["num_crossings"],[int],[(1,4)]))
        print(case2)
